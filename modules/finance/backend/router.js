...
router.get('/invoices/:id', authenticate, requireRole(STAFF_ROLES), checkPolicy('Invoice', 'read'), ctrl.getInvoiceById);
router.post('/invoices/generate', authenticate, requireRole(MGT_ROLES), ctrl.generateInvoice);
router.patch('/invoices/:id/status', authenticate, requireRole(MGT_ROLES), checkPolicy('Invoice', 'update'), ctrl.updateInvoiceStatus);

// Payments (Phiếu Thu)
router.post('/payments', authenticate, requireRole(STAFF_ROLES), ctrl.recordPayment);

// Credits (Ví dư & Hoàn trả)
router.get('/contracts/:id/credits', authenticate, requireRole(STAFF_ROLES), checkPolicy('Contract', 'read'), ctrl.getContractCreditDetails);
router.post('/contracts/:id/credits/refund', authenticate, requireRole(MGT_ROLES), checkPolicy('Contract', 'update'), ctrl.refundContractCredit);
